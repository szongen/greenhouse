...
#define AUTH_INFO	"7O9VpkW=3pjugMZluuQAYHH=E8Q="

#define DEVID		"918176186"
...
//	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Auto\":%d,", Auto_Flag);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Alarm\":%d,", MQ2_ADC_Alarm);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Temp\":%d,", DHT11_1.Temp);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Humidity\":%d,", DHT11_1.Humidity);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"MQ2\":%0.3f,", MQ2_ADC);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"relay\":%d", led_status.Led2Sta);
	strcat(buf, text);
...
//			}
//		}
		if(strstr((char *)req_payload, "Water"))
		{
			if(num == 1)
			{
				Led2_Set(LED_ON);
